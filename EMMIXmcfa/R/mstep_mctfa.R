...
for (i in 1 : g) {
  ...
  twY_AXI <- sweep(Y_AXI, 1, tauW[, i], '*')
  xi[, i] <- XI + t(gamma) %*% as.matrix(colSums(twY_AXI)) / twi
  omega[,, i] <- (diag(q) - t(gamma) %*% A) %*% omega[,, i] +
                      t(gamma) %*% t(Y_AXI) %*% twY_AXI %*% gamma/ti -
                      (XI - xi[, i]) %*% t(XI - xi[, i])
  A1 <- A1 + colSums(twY) %*% t(XI) + t(Y) %*% twY_AXI %*% gamma
  A2 <- A2 + omega[,, i]* ti + xi[, i] %*% t(xi[, i]) * twi
  Di <- Di + colSums(Y * twY) #diag(t(Y) %*% twY)
  pivec[i] <- ti / n
  if (df_update) {
    f <- function(x) {
